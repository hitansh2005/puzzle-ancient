...
function handleTouchStart(e, piece, fromPile = false) {
  if (paused) return;
  if (!gameStarted) startTimer();
  touchPiece = piece;
  ...
  touchClone.style.height = e.currentTarget.offsetHeight + "px";
...
// Patch touch listeners into createPieceElement
const originalCreatePieceElement = createPieceElement;
createPieceElement = function(piece, index, fromPile = false) {
  const wrapper = originalCreatePieceElement(piece, index, fromPile);
  wrapper.addEventListener('touchstart', (e) => handleTouchStart(e, piece, fromPile), { passive: false });
